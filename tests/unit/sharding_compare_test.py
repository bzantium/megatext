...
from megatext.utils.sharding import clear_input_shardings_dump
# import optax

from megatext.layers import quantizations
from megatext.models import models
from megatext.optimizers import optimizers
from megatext.trainers.pre_train.train_compile import get_shaped_inputs, get_topology_mesh, validate_config
...
@pytest.mark.parametrize("model, topology, num_slice", TEST_CASES)
def test_sharding_dump_for_model(model: str, topology: str, num_slice: str) -> None:
  """
  ...
  base_path = os.path.join(root_dir, model, topology, f"slice_{num_slice}")
  ...
  if not os.path.exists(named_json_path):
    pytest.skip(f"Missing named_shardings.json for {model} {topology} slice {num_slice}")
    return
  if not os.path.exists(logical_json_path):
    pytest.skip(f"Missing logical_shardings.json for {model} {topology} slice {num_slice}")
    return
  if not os.path.exists(input_json_path):
    pytest.skip(f"Missing input_shardings.json for {model} {topology} slice {num_slice}")
    return
  ...
  if not named_match:
    print(f"\n[FAIL] Physical Sharding Mismatch: {model} {topology} slice {num_slice}", flush=True)
    compare_sharding_jsons(expected_named, "Expected (Physical)", actual_named, "Actual (Physical)")
    error_messages.append(f" Physical sharding mismatch for {model} on {topology} slice {num_slice}")
  ...
  if not logical_match:
    print(f"\n[FAIL] Logical Sharding Mismatch: {model} {topology} slice {num_slice}", flush=True)
    compare_sharding_jsons(expected_logical, "Expected (Logical)", actual_logical, "Actual (Logical)")
    error_messages.append(f"Logical sharding mismatch for {model} on {topology} slice {num_slice}")
  ...
  if not input_match:
    print(f"\n[FAIL] Input Sharding Mismatch: {model} {topology} slice {num_slice}", flush=True)
    # compare_sharding_jsons(expected_input, "Expected (Input)", actual_input, "Actual (Input)")
    error_messages.append(f"Input sharding mismatch for {model} on {topology} slice {num_slice}")
...
def abstract_state_and_shardings(request):
  ...
  model, topology, num_slice = request.param
  print(f"Testing model: {model}, topology: {topology}, num_slices: {num_slice}", flush=True)
  params = [
      "/deps/MaxText/tests/unit/sharding_compare_test",
      get_test_config_path(),
      f"compile_topology={topology}",
  ...
  # Get abstract state and physical shardings from megatext_utils
  abstract_state, _, state_mesh_shardings = megatext_utils.get_abstract_state(
      model, tx, config, rng, topology_mesh, is_training=True
  )

  # Get logical shardings from megatext_utils
  logical_shardings = megatext_utils.get_logical_annotations(model, tx, config, rng, topology_mesh, is_training=True)

  return model, topology, num_slice, abstract_state, state_mesh_shardings, logical_shardings
...
class TestGetAbstractState:
  ...
  def test_get_abstract_state_sharding(self, abstract_state_and_shardings):  # pylint: disable=redefined-outer-name
    ...
    model, topology, num_slice, abstract_state, state_mesh_shardings, logical_shardings = (
