...
def torch_chunk_gated_delta_rule(
  ...
  Returns:
    A tuple containing the attention output tensor and the final hidden state
    (if requested).
  """
...
class Qwen3NextRotaryEmbedding_PT(nn.Module):
  ...
  def compute_default_rope_parameters(
    ...
    Args:
        ...
        device (`torch.device`): The device to use for initialization of the
          inverse frequencies.
        seq_len (`int`, *optional*): The current sequence length. Unused for
          this type of RoPE.

    Returns:
        Tuple of (`torch.Tensor`, `float`), containing the inverse frequencies
        for the RoPE embeddings and the
        post-processing scaling factor applied to the computed cos/sin (unused
        in this type of RoPE).
    """
...
class Qwen3NextSparseMoeBlock_PT(nn.Module):
  ...
  def route_tokens_to_experts(self, hidden_states, router_logits):
    ...
    Returns:
      A tuple containing the indices of the selected experts and their
...
class Qwen3NextRMSNormGated_PT(nn.Module):
  ...
  If a gate tensor is provided, the normalized output is element-wise multiplied
  by SiLU(gate).
  """
...
class Qwen3NextGatedDeltaNet_PT(nn.Module):
  ...
  This module implements a form of linear attention with gating and
...
class TestQwen3Next(unittest.TestCase):
  ...
  def test_qwen3_next_sparse_moe_block(self):
    ...
    This test passes by setting `sparse_matmul=False` in the config, which
    forces the
  ...
  def _run_full_attention_jax_vs_pytorch_attention(self, attention_type):
    ...
    self.assertEqual(
        pt_out_np.shape,
        jax_out_np.shape,
        "Shape mismatch for jax and pytorch impls",
    )
    np.testing.assert_allclose(
        pt_out_np,
        jax_out_np,
