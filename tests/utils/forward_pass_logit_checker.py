...
# This script, forward_pass_logits_checker.py, verifies the correctness of logits generated by the Megatext or HuggingFace
...
#   tests/assets/golden_logits/golden_data_<model>.jsonl
# For example:
...
#
# The script runs a forward pass using the Megatext implementation and compares the resulting logits against the golden
...
"""Check if the logits generated by a model's src/megatext/HF implementation matches golden logits for the same inputs"""
...
from megatext.utils.constants import MEGATEXT_TEST_ASSETS_ROOT
...
def get_data(golden_data_point, config):
  ...
  # Pad ids to max_target_length. Megatext expects 0 for padding.
...
def main(config, test_args):  # pylint: disable=W0621
  """Test the Whole Model of model"""
  if not test_args.run_hf_model:
    """Comparing megatext/huggingface model with pre-loaded golden logitis"""
    ...
    if test_args.golden_logits_path == "":
      input_golden_data_path = os.path.join(
          ...
          "golden_logits",
          f"golden_data_{config.model}.jsonl",
      )
    else:
    ...
    for golden_data_index, golden_data_point in enumerate(golden_data):
      ...
      max_logging.log("megatext forward pass")
    ...
    """Comparing megatext model with HF model on-the-fly"""
    if test_args.hf_model_path == "":
      raise ValueError("run_hf_model requires hf_model_path")

    hf_token = config.hf_access_token
    # Map MegaText config.dtype to PyTorch dtype
    dtype_mapping = {
    ...
    # megatext model prefix, use eos token as pad token
    pad_token_prefixes = ["llama3.1", "mixtral"]
    if any(config.model.startswith(prefix) for prefix in pad_token_prefixes):
    ...
    for input_text in prompts:
      ...
      # --- Megatext Forward Pass ---
      mt_logits_jax = megatext_model.apply(
          megatext_state.params,
          mt_ids,
      ...
      mt_last_token_logits = mt_logits_torch[:, -1, :]  # Megatext output already sliced to actual_seq_len

      tokens_megatext = get_top_k_tokens_scores(mt_last_token_logits, tokenizer, k=10, description="MegaText model")
      tokens_hf = get_top_k_tokens_scores(hf_last_token_logits, tokenizer, k=10, description="HF model")
      compare_top_tokens(converted_tokens=tokens_megatext, golden_tokens=tokens_hf)
...
if __name__ == "__main__":
  ...
  # Reconstruct model_args (script name + the args Megatext needs)
