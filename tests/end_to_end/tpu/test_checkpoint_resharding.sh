...
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"/base.yaml run_name=$RUN_NAME steps=101\
...
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"/base.yaml run_name=$RUN_NAME steps=102\
...
python3 tests/end_to_end/tpu/eval_assert.py checkpoint_save_restore metrics.txt learning/loss
