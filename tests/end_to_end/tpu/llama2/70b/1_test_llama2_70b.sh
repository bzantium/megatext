...
# Example Usage: export BASE_OUTPUT_PATH=/path/to/GCS/bucket; bash tests/end_to_end/tpu/llama2/70b/1_test_llama2_70b.sh
# Use the same BASE_OUTPUT_PATH as tests/end_to_end/tpu/llama2/70b/2_test_llama2_70b.sh.
...
JAX_PLATFORMS=cpu python3 -m megatext.utils.generate_param_only_checkpoint "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"/base.yaml async_checkpointing=false base_output_directory=${BASE_OUTPUT_PATH} load_parameters_path=${CONVERTED_CHECKPOINT} run_name=${RUN_NAME} model_name='llama2-70b' force_unroll=true skip_jax_distributed_system=true
