...
# This file, combined with step 2 in the same directory, demonstrates converting a Llama3.3-70B-Instruct checkpoint from Meta and running various MaxText operations on it.
...
# Example Usage: export BASE_OUTPUT_PATH=/path/to/GCS/bucket; bash tests/end_to_end/tpu/llama3.3/70b/1_test_llama3.3_70b.sh
# Use the same BASE_OUTPUT_PATH as tests/end_to_end/tpu/llama3.3/70b/2_test_llama3.3_70b.sh.
...
# the subfolders names aka RUN_NAMEs are static. Please remember to change BASE_OUTPUT_PATH across different runs.ex

set -ex
MODEL_VARIATION='llama3.3-70b'
...
export META_CHECKPOINT_PATH=gs://maxtext-llama/llama3.3_70b_instruct/meta-ckpt
...
JAX_PLATFORMS=cpu python3 -m maxtext.utils.generate_param_only_checkpoint "${MAXTEXT_CONFIGS_DIR:-${MAXTEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml async_checkpointing=false base_output_directory=${BASE_OUTPUT_PATH} load_parameters_path=${CONVERTED_CHECKPOINT} run_name=${RUN_NAME} model_name=${MODEL_VARIATION} force_unroll=true
