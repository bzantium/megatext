...
python3 -m megatext.checkpoint_conversion.standalone_scripts.convert_gpt3_ckpt_from_paxml --paxml-ckpt-path=${PAXML_CKPT_PATH} --maxtext-model-name=gpt3-52k --run-name=${RUN_NAME} --base-output-directory=${OUTPUT_PATH}

# Run gpt3-52k with the converted ckpt
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"/base.yaml run_name=${RUN_NAME} model_name=gpt3-52k\
