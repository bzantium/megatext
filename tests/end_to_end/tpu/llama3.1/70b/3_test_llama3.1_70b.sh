...
export TOKENIZER="${MEGATEXT_ASSETS_ROOT:-${MEGATEXT_PKG_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/assets/tokenizers}}"/tokenizer_llama3.tiktoken
...
export GOLDEN_LOGITS="${MEGATEXT_TEST_ASSETS_ROOT:-${MEGATEXT_REPO_ROOT:-$PWD}/tests/assets}"/golden_logits/golden_data_deepseek_r1_distill_llama3.1_70b.jsonl
...
JAX_PLATFORMS=cpu python3 -m megatext.checkpoint_conversion.standalone_scripts.llama_mistral_mixtral_orbax_to_hf "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml base_output_directory=gs://runner-maxtext-logs load_parameters_path=${CHECKPOINT_TPU_SCANNED}/0/items run_name=convert_to_hf model_name=${MODEL_SIZE} hf_model_path=$CHECKPOINT_TPU_CONVERTED_BACK
...
JAX_PLATFORMS=cpu python3 -m megatext.utils.generate_param_only_checkpoint "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml async_checkpointing=false base_output_directory=${BASE_OUTPUT_PATH} load_parameters_path=${CHECKPOINT_TPU_SCANNED}/0/items run_name=${RUN_NAME} model_name=${MODEL_SIZE} force_unroll=true
