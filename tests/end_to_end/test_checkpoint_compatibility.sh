...
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml run_name=$RUN_NAME steps=3 ${model_params}\
...
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml run_name=$RUN_NAME steps=5 ${model_params}\
...
python3 -m megatext.trainers.pre_train.train "${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}"//base.yaml run_name=$RUN_NAME steps=7 ${model_params}\
    max_target_length=128 per_device_batch_size=1\
...
python3 tests/end_to_end/tpu/eval_assert.py test_start_step run_2_metrics.txt 3.0
python3 tests/end_to_end/tpu/eval_assert.py test_start_step run_3_metrics.txt 5.0
