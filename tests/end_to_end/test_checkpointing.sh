...
CMD1="python3 -m megatext.trainers.pre_train.train ${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}/base.yaml run_name=$RUN_NAME steps=5 max_target_length=128 per_device_batch_size=1\
...
CMD2="python3 -m megatext.trainers.pre_train.train ${MEGATEXT_CONFIGS_DIR:-${MEGATEXT_REPO_ROOT:-$PWD}/src/maxtext/configs}/base.yaml run_name=$RUN_NAME steps=10 max_target_length=128 per_device_batch_size=1\
...
python3 tests/end_to_end/tpu/eval_assert.py $eval_metrics metrics.txt learning/loss
