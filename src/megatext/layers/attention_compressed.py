...
from megatext.layers.embeddings import DeepSeekV4RotaryEmbedding
from megatext.layers.initializers import nd_dense_init, NdInitializer, variable_to_logically_partitioned
from megatext.layers.linears import DenseGeneral, DeepSeekV4GroupedLinear
...
class CompressedAttention(Attention):
  ...
  def __init__(
    ...
    Args:
      (See megatext.layers.attentions.Attention for standard attention arguments)
  ...
  def _init_projections(self, inputs_q_shape: Tuple, inputs_kv_shape: Tuple) -> None:
    ...
    # CSA / HCA layers use compressed_rope_theta (160000).
    # Sliding window prefix layers use rope_theta (10000).
    rope_theta = self.config.compressed_rope_theta if self.compress_ratio > 0 else self.config.rope_theta
