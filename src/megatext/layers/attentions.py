...
from megatext.layers.initializers import nd_dense_init, NdInitializer, variable_to_logically_partitioned, default_bias_init
from megatext.layers.linears import DenseGeneral, canonicalize_tuple, normalize_axes
from megatext.layers.normalizations import RMSNorm, Qwen3NextRMSNorm, GlobalRMSNorm
from megatext.layers.quantizations import AqtQuantization as Quant
from megatext.inference import kvcache, page_manager, paged_attention
...
class Attention(nnx.Module):
  ...
  def __init__(
    ...
    if self.use_qk_norm and not is_llama4_decoder_block:
      ...
      use_global_qk_norm = self.config.decoder_block in (DecoderBlockType.OLMO3, DecoderBlockType.OLMO3.value)
    ...
    if self.use_v_norm:
      with_scale = getattr(self.config, "v_norm_with_scale", True)
  ...
  def init_rotary_embedding(self):
    ...
    if self.is_vision:
      if decoder_block == DecoderBlockType.QWEN3.value:
        ...
        raise ValueError(f"Unsupported model type for vision rotary embedding: {self.config.model}")
  ...
  def __call__(
    ...
    Args:
      ...
      kv_cache: Optional KV cache input for decode-time execution.
