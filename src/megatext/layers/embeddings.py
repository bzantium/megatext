...
class DeepSeekV4RotaryEmbedding(RotaryEmbedding):
  ...
  def __call__(
    ...
    if reverse:
      sin = -sin
    return _apply_interleaved_rotary_pos_emb(inputs, cos, sin, unsqueeze_dim=unsqueeze_dim)


def _rotate_half_interleaved(x: jax.Array) -> jax.Array:
...
def _apply_interleaved_rotary_pos_emb(
    x: jax.Array,
    cos: jax.Array,
    sin: jax.Array,
    unsqueeze_dim: int | None = 1,
  ...
  rotated = ((rope_f32 * cos) + (_rotate_half_interleaved(rope_f32) * sin)).astype(x.dtype)
