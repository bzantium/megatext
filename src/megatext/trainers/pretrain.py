...
# See github.com/google/megatext/issues/20 for more
...
from megatext.layers.multi_token_prediction import calculate_mtp_acceptance_rate, calculate_mtp_loss
from megatext.common import checkpointing, profiler
...
from megatext.common.gcloud_stub import cloud_diagnostics as _cloud_diag, is_decoupled
from megatext.common.gcloud_stub import vertex_tensorboard_modules
from megatext.common.metric_logger import MetricLogger, record_activation_metrics
...
def train_loop(config, recorder, state=None):
  ...
  completed_steps = get_first_step(state)
  prof = profiler.Profiler(config, offset_step=completed_steps)
  ...
  try:
    ...
    for step in range(completed_steps + 1, config.steps + 1):
      ...
      if config.dump_hlo and step == (config.dump_step if config.dump_step >= 0 else completed_steps + 1):
      ...
      if config.eval_interval > 0 and step > completed_steps + 1 and step % config.eval_interval == 0:
    ...
    if config.save_checkpoint_on_completion:
      state_to_save = state
      checkpointing.maybe_save_checkpoint(checkpoint_manager, state_to_save, config, data_iterator, force=True)
