...
def get_topology_mesh(config):
  ...
  topology_device_mesh = megatext_utils.create_device_mesh(config, topology_devices)
...
def get_shaped_inputs(topology_mesh, config):
  ...
  logical_annotations = megatext_utils.get_logical_annotations(model, tx, config, example_rng, topology_mesh)
...
def jit_and_compile(
  ...
  with jax.set_mesh(mesh), mesh, logical_axis_rules:
    ...
    megatext_utils.maybe_dump_jaxpr(config, jitted, func_input_args)
