...
from megatext.checkpoint_conversion.utils.param_mapping import (
    HOOK_FNS,
    PARAM_MAPPING,
)
from megatext.checkpoint_conversion.utils.hf_shape import HF_SHAPE
from megatext.checkpoint_conversion.utils.hf_model_configs import HF_MODEL_CONFIGS
from megatext.checkpoint_conversion.utils.utils import (
...
def _get_model_mappings(
    model: str, scan_layers: bool, hf_config_dict: dict, maxtext_config: pyconfig.HyperParameters
):
  """Retrieves parameter, shape, and hook function mappings for the model.

  Args:
    model: The name of the model (e.g., "gemma2-2b").
  ...
  Raises:
    ValueError: If mappings for the specified `model` are not found.
  """
  if model not in PARAM_MAPPING or model not in HF_SHAPE or model not in HOOK_FNS:
    raise ValueError(f"Mappings not found for model: {model}. Available PARAM_MAPPING keys: {PARAM_MAPPING.keys()}")

  return {
      "param_mapping": PARAM_MAPPING[model](hf_config_dict, maxtext_config, scan_layers),
      "shape_mapping": HF_SHAPE[model](hf_config_dict),
      "hook_fn_mapping": HOOK_FNS[model](hf_config_dict, maxtext_config, scan_layers, saving_to_hf=True),
  }
...
def _validate_or_update_architecture(hf_config, max_config, override: bool):
  ...
  for hf_attr, mt_attr in attributes_to_check:
    ...
    if max_config.decoder_block == "gemma2" and hf_attr == "num_hidden_layers":
...
def main(argv: Sequence[str]) -> None:
  ...
  if model_key not in HF_IDS:
    raise ValueError(f"Unsupported model name: {config.model}. Supported models are: {list(HF_IDS.keys())}")
