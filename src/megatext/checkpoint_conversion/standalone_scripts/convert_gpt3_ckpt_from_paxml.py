...
python3 -m megatext.checkpoint_conversion.standalone_scripts.convert_gpt3_ckpt_from_paxml \
...
from megatext.models.models import transformer_as_linen
...
def convert(paxml_ckpt_path, maxtext_model, base_output_directory, run_name):
  """convert ckpt."""

  base_args = [
      "",
      os.path.join(MEGATEXT_PKG_DIR, "configs", "base.yml"),  # base arg
      "per_device_batch_size=1",
      "ici_fsdp_parallelism=-1",
      "ici_tensor_parallelism=1",
      f"model={maxtext_model}",
      f"run_name={run_name}",
  ...
  learning_rate_schedule = megatext_utils.create_learning_rate_schedule(cfg)
  ...
  state, _, _, _ = megatext_utils.setup_training_state(model, None, tx, cfg, init_rng, mesh, checkpoint_manager)
...
if __name__ == "__main__":
  ...
  convert(args.paxml_ckpt_path, args.maxtext_model, args.base_output_directory, args.run_name)
