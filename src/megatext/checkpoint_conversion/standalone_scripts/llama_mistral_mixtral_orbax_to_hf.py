...
Example cmd:
To save a ckpt
python3 -m megatext.checkpoint_conversion.standalone_scripts.llama_or_mistral_ckpt --base-model-path <path/to/meta/ckpt> \
    --maxtext-model-path <GCS/path/to/save/new/maxtext/ckpt> --model-size llama2-7b

python3 -m megatext.checkpoint_conversion.standalone_scripts.llama_mistral_mixtral_orbax_to_hf src/maxtext/configs/base.yml
            base_output_directory=path/to/saving/intermediate_MaxText_files
            load_parameters_path=/path/to/src/maxtext/checkpoint run_name=<your run name> model=<llama2 or mistral>
            hardware=gpu
...
from megatext.utils.generate_param_only_checkpoint import _read_train_checkpoint
from megatext.checkpoint_conversion.standalone_scripts import llama_or_mistral_ckpt
from megatext.common import checkpointing
from megatext.utils import max_logging
from megatext.utils import megatext_utils
from megatext.utils.max_utils import unpermute_from_match_maxtext_rope
...
def convert_orbax_hf(hf_model_path, config):
  ...
  new_hf_model_params = convert_state_to_hf(training_state, config.model)
