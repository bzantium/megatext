...
JAX_PLATFORMS=cpu python3 -m megatext.checkpoint_conversion.compare_hf_ckpt src/maxtext/configs/base.yml \
    model=<maxtext_model> \
...
from megatext.checkpoint_conversion.utils.utils import print_ram_usage, load_hf_dict_from_transformers
...
def main(args: Sequence[str], test_args: argparse.Namespace) -> None:
  ...
  if test_args.reference_path:
    ...
    model = config.model
    if model not in HF_IDS:
      raise ValueError(f"Unsupported model name: {model}. " f"Supported: {list(HF_IDS.keys())}")

    model_id = HF_IDS[model]
