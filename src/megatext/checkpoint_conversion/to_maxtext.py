...
Key Parameters (to be set in the config file or as command-line overrides):
  model: (Required) The name of the model to convert (e.g., "gemma2-2b").
              Must be a key in `megatext.utils.globals.HF_IDS`.
  ...
  --hf_model_path: (Optional) Specifies a local or remote directory containing the model weights.
      ...
      (e.g., openai/gpt-oss-20b; see `HF_IDS[model]` in `megatext.utils.globals`).
...
Example Usage:
  To convert a gemma2-2b model and save it to a specific directory:
   ...
   python -m megatext.checkpoint_conversion.to_maxtext \
    maxtext/configs/base.yml model="gemma2-2b" \
  ...
  To convert a 70B model with minimal RAM usage:
   ...
   python -m megatext.checkpoint_conversion.to_maxtext \
    maxtext/configs/base.yml model="llama3.1-70b" \
...
from megatext.configs.types import DType
from megatext.common.common_types import MODEL_MODE_TRAIN
from megatext.checkpoint_conversion.standalone_scripts.llama_or_mistral_ckpt import save_weights_to_checkpoint
from megatext.checkpoint_conversion.utils.hf_model_configs import HF_MODEL_CONFIGS
from megatext.checkpoint_conversion.utils.param_mapping import HOOK_FNS, PARAM_MAPPING
from megatext.checkpoint_conversion.utils.utils import MemoryMonitorTqdm, apply_hook_fns, load_hf_dict_from_transformers, load_hf_dict_from_safetensors, print_peak_memory, print_ram_usage, validate_and_filter_param_map_keys
...
from megatext.utils import max_logging, max_utils, megatext_utils
from megatext.utils.globals import HF_IDS
import numpy as np
...
def get_maxtext_model_info(config):
  ...
  abstract_params_tree = megatext_utils.get_abstract_param(maxtext_model_flax, config)["params"]
...
def main(
  ...
  for i, arg in enumerate(args):
    if arg.startswith("model="):
      model_arg = args[i].split("=")[1]
      model_original = model_arg
      if "-Instruct" in model_arg:
        max_logging.log("Warning: You want an Instruct version, so we are using the base model architecture instead.")
        model_arg = model_arg.replace("-Instruct", "")
        args[i] = f"model={model_arg}"
      break

  # check the supported model ids
  if model_original not in HF_IDS:
    raise ValueError(
        f"Unsupported model name: {model_original}.\
                      Supported models are: {list(HF_IDS.keys())}"
    )

  model_id = hf_model_path or HF_IDS[model_original]
...
if __name__ == "__main__":
  ...
  # If not specified, default to megatext.utils.globals.HF_IDS[model]
  parser.add_argument(
      "--hf_model_path",
      type=str,
      required=False,
