...
from megatext.common.common_types import Config, AttentionType, MODEL_MODE_PREFILL
...
from megatext.layers.linears import DenseGeneral, MlpBlock, Dropout
...
def get_query_pre_attn_scalar(config) -> float:
  ...
  if config.decoder_block == "gemma3":
    if config.model in ["gemma3-27b"]:
      return (config.base_emb_dim // config.base_num_query_heads) ** -0.5
    else:
      return config.head_dim**-0.5
  else:
    raise ValueError(f"Unsupported decoder_block: {config.decoder_block}")
