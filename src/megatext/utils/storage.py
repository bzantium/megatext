...
from megatext.common.gcloud_stub import is_decoupled, gcs_storage
...
def mkdir_and_check_permissions(path: str | epath.Path) -> epath.Path:
  ...
  if path.as_posix().startswith("gs://"):
    # GCS has no directories — just validate the bucket exists.
    if len(path.parts) < 3:
  ...
  # Local filesystem: create directory and verify write permissions.
  path.mkdir(exist_ok=True, parents=True)
  if not path.exists():
