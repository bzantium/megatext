...
numbers for a specific model. Example:
  python3 -m megatext.muon_utils qwen3-4b True
"""
...
def get_model_mdn(model, scan_layers=True, verbose=False):
  ...
  Args:
    model: The name of the model to be initialized.
...
if __name__ == "__main__":
  if len(sys.argv) != 3:
    print("Usage: python3 -m megatext.utils.muon_utils <model> <scan_layers:True/False>")
    sys.exit(1)
  model_arg = sys.argv[1]
  scan_layers_arg = sys.argv[2].lower() == "true"
  get_model_mdn(model_arg, scan_layers_arg, verbose=True)
