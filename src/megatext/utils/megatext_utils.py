...
from megatext.common.common_types import DecoderBlockType, MODEL_MODE_PREFILL, MODEL_MODE_AUTOREGRESSIVE
from megatext.configs import types
from megatext.inference.page_manager import PageState
...
def get_input_data_sharding(config, mesh):
  max_logging.log(
      "WARNING: Function megatext_utils.get_input_data_sharding is deprecated. Please use sharding.get_input_data_sharding."
  )
...
def assert_params_sufficiently_sharded(params, mesh, tolerance):
  max_logging.log(
      "WARNING: Function megatext_utils.assert_params_sufficiently_sharded is deprecated."
...
def add_data_to_sharding(mesh, path, aval, shardings):
  max_logging.log(
      "WARNING: Function megatext_utils.add_data_to_sharding is deprecated. Please use sharding.add_data_to_sharding."
  )
...
def maybe_update_params_sharding_with_opt(config, state_mesh_shardings):
  max_logging.log(
      "WARNING: Function megatext_utils.maybe_update_params_sharding_with_opt is deprecated."
...
def all_gather_over_fsdp(variables, sharding_info, mesh, logical_axis_rules, shard_mode):
  max_logging.log(
      "WARNING: Function megatext_utils.all_gather_over_fsdp is deprecated. Please use sharding.all_gather_over_fsdp."
  )
