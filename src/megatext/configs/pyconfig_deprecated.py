...
from megatext.utils.globals import MEGATEXT_ASSETS_ROOT, MEGATEXT_REPO_ROOT, MEGATEXT_PKG_DIR
...
def validate_keys(keys):
  ...
  if keys["use_multimodal"]:
    validate_multimodal_model(keys["model"])
...
def validate_model(s: str) -> bool:
  ...
  if s not in valid_models:
    raise ValueError(f"Invalid model name was passed. Got {s}, Valid options {valid_models}")


def validate_multimodal_model(s: str) -> bool:
  valid_models = (
      "gemma3-4b",
      "gemma3-12b",
      "gemma3-27b",
  ...
  if s not in valid_models:
    raise ValueError(
        f"Invalid multimodal model name was passed. Got {s}. Valid options which support multimodal inputs are: {valid_models}"
    )
...
class _HyperParameters:
  ...
  def __init__(self, argv: list[str], **kwargs):
    ...
    if not os.path.isfile(raw_keys["tokenizer_path"]):
      ...
      for search_root in (
          ...
          os.path.dirname(MEGATEXT_ASSETS_ROOT),
          os.path.join(MEGATEXT_REPO_ROOT, "assets"),
          MEGATEXT_REPO_ROOT,
          os.path.join(MEGATEXT_REPO_ROOT, "src", "MaxText"),
          MEGATEXT_PKG_DIR,
  ...
  def user_init(raw_keys):
    ...
    if raw_keys["run_name"] == "":
      ...
      if raw_keys["run_name"] == "":
        ...
        raw_keys["run_name"] = f'{raw_keys["model"]}_{timestamp}'
  ...
  def update_model_vars(base_config_path, raw_keys, config_name: str, keys_from_env_and_command_line):
    ...
    validate_model(raw_keys["model"])
    max_logging.log(f"Running Model: {raw_keys['model']}")

    updated_keys = []
    if raw_keys["model"] != "default":
      model = raw_keys["model"]
      ...
      file_path = os.path.join(os.path.dirname(base_config_path), "models", f"{model}.yml")
      if not os.path.isfile(file_path):
        dir_path = os.path.dirname(os.path.realpath(__file__))
        file_path = os.path.join(dir_path, "configs", "models", f"{model}.yml")
