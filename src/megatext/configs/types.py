...
class MMapArecordDataset(BaseModel):
  """Configuration specific to mmap/arecord data pipeline (dataset_type=mmap, arecord, or fixed_arecord)."""

  data_split: str = Field(
      "1,0,0",
      description="Train,val,test proportions (e.g. '99,1,0').",
  )
  max_chunks_per_sample: int = Field(
      0,
      description="Max documents per packed sample (0 = unlimited).",
  )
  add_extra_token: bool = Field(
      True,
      description="Add extra token for input/target shift.",
  )
  data_cache_dir: str = Field(
      "",
      description="Directory for cached sample indices. Empty = auto (next to data).",
  )
  ...
  def set_derived_and_validate_values(self) -> "MegaTextConfig":
    ...
    if not self.run_name:
      if os.environ.get("JOBSET_NAME"):
        ...
        self.run_name = f'{self.model}_{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")}'
      ...
      tokenizer_path = next(
          filter(
              ...
              (
                  ...
                  os.path.join(MEGATEXT_ASSETS_ROOT, "tokenizers", tokenizer_path),
              ),
          ),
          tokenizer_path,
      )
    ...
    if self.use_multimodal:
      ...
      if self.model not in valid_mm_models and self.model != "default":
        raise ValueError(f"Multimodal is only supported for {valid_mm_models}, not {self.model}")
    ...
    if self.dataset_type in (DatasetType.MMAP, DatasetType.ARECORD, DatasetType.FIXED_ARECORD):
      if not self.dataset_path:
        raise ValueError(f"dataset_path can't be empty when dataset_type={self.dataset_type.value}")
    ...
    if self.opt_type == "muon" and self.decoder_block not in [
      ...
      raise ValueError(
          ...
          f"`python3 -m megatext.utils.muon_utils {self.model} True`"
      )
