...
# pytype: skip-file
"""Pydantic-based configuration management for MegaText."""
import logging
import os
import sys
...
def _module_from_path(path: str) -> str | None:
  ...
  pkg_parent = os.path.realpath(os.path.dirname(MEGATEXT_PKG_DIR))
...
def _resolve_or_infer_config(argv: list[str]) -> tuple[str, list[str]]:
  ...
  config_path = os.path.join(MEGATEXT_CONFIGS_DIR, _CONFIG_FILE_MAPPING[module])
  ...
  if _BASE_CONFIG_ATTR in cfg:
    ...
    if not os.path.isabs(base_path):
      ...
      if not os.path.isfile(loaded_parent_config_filename):
        loaded_parent_config_filename = os.path.join(MEGATEXT_CONFIGS_DIR, base_path)
    else:
...
def _prepare_for_pydantic(raw_keys: dict[str, Any]) -> dict[str, Any]:
  ...
  pydantic_kwargs = {}
  valid_fields = types.MegaTextConfig.model_fields.keys()
    ...
    if key == "muon_consistent_rms":
      if value is None or (isinstance(value, str) and value.lower() == "none"):
        ...
        try:
          ...
          raise ValueError("muon_consistent_rms should be null or float") from e
...
class HyperParameters:
  ...
  def __init__(self, pydantic_config: types.MegaTextConfig):
...
def initialize_pydantic(argv: list[str], **kwargs) -> MegaTextConfig:
  """Initializes the configuration by loading YAML files, and applying CLI, env, and kwarg overrides.
  Returns pydantic MegaTextConfig class whereas `initialize` returns the og `HyperParameters`
  """
  ...
  model = temp_cfg.get("model", "default")
  # The architecture for -Instruct v/s base models are the same, so for identifying the
  # architecture we replace "-Instruct" from the model and get the base model name
  model = model.replace("-Instruct", "") if "-Instruct" in model else model
  model_cfg = {}
  if model != "default":
    ...
    if os.path.exists(model_config_path):
      ...
      logger.warning("Model config for '%s' not found at %s", model, model_config_path)
  ...
  pydantic_config = types.MegaTextConfig(**pydantic_kwargs)
  config = HyperParameters(pydantic_config)
