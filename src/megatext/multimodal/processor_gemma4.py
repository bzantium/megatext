...
# Use rectangular dimensions to yield exactly 2520 patches (280 soft tokens with 3x3 pooling).
...
def preprocess_mm_data_gemma4(images):
  ...
  for img in images_in:
    ...
    # Gemma4 expects normalized image inputs in [0, 1].
    processed = np.asarray(resized_pil_img, dtype=np.float32) / 255.0
    images_out.append(processed)
...
def get_image_offsets_gemma4(processor_output: mm_utils.PreprocessorOutput | None):
  ...
  # -1 because one placeholder token already exists in the input text.
...
def add_extra_tokens_for_images_gemma4(
  ...
  """Replaces image placeholder tokens with the full sequence of Gemma4 image tokens."""
  num_patches = (GEMMA4_IMAGE_HEIGHT // GEMMA4_PATCH_SIZE) * (GEMMA4_IMAGE_WIDTH // GEMMA4_PATCH_SIZE)
  num_soft_tokens = num_patches // (GEMMA4_POOLING_KERNEL**2)
...
def get_dummy_image_shape_for_init_gemma4(batch_size=1, num_image_per_sequence=1):
  """Returns the shape of the dummy image for Gemma4 model initialization."""
  return (
      batch_size,
